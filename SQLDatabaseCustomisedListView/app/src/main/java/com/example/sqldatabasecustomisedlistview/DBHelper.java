...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<StudentModel> getAllStudents() {
        ...
        if (cursorCourses.moveToFirst()) {
            do {
                System.out.println(cursorCourses.getInt(0));
    ...
    {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(STUDENT_TABLE, "StudentRollNumber = ?",new String[] {rollno});


    }
    public boolean updateStudent(StudentModel STUDENTModel,String rollno) {
        SQLiteDatabase db = this.getWritableDatabase();
        //Hash map, as we did in bundles
        ContentValues cv = new ContentValues();
        ...
        cv.put(STUDENT_ROLL, STUDENTModel.getRollNmber());
        cv.put(STUDENT_ENROLL, STUDENTModel.isEnroll());
        db.update(STUDENT_TABLE, cv, "StudentRollNumber = ?",new String[] { rollno });
        return true;
    }
